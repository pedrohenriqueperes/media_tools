...
Django 6 web app for media processing. Users upload files and choose a job type; processing runs synchronously in a daemon thread (no Celery in dev). Auth is e-mail only via django-allauth. Frontend uses Bootstrap 5 + Plus Jakarta Sans + Remix Icons with a custom design system defined entirely in `templates/base.html`.
...
3. `process_job_async(job.pk)` spawns a daemon thread that calls `process_job()` in `core/tasks.py`.
4. The thread sets `status='processing'`, runs the appropriate private function, then saves `status='done'` or `status='error'` in a `finally` block.
